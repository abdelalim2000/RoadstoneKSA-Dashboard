...
class TireController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Tire::with(['tire_features', 'tire_designs', 'car_models', 'car_type'])->select(sprintf('%s.*', (new Tire())->table));
    ...
    public function create()
    {
        ...
        $tire_features = TireFeature::pluck('name', 'id');

        $tire_designs = TireDesign::pluck('name', 'id');

        $car_models = CarModel::pluck('name', 'id');

        $car_types = CarType::pluck('name', 'id')->prepend(trans('global.pleaseSelect'), '');
    ...
    public function store(StoreTireRequest $request)
    {
        ...
        if ($request->input('breadcrumb', false)) {
            $tire->addMedia(storage_path('tmp/uploads/' . basename($request->input('breadcrumb'))))->toMediaCollection('breadcrumb');
        }

        if ($request->input('thumb', false)) {
            $tire->addMedia(storage_path('tmp/uploads/' . basename($request->input('thumb'))))->toMediaCollection('thumb');
        }
        ...
        foreach ($request->input('images', []) as $file) {
            $tire->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('images');
        }
