...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.tireSize.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.tire-sizes.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ratio') ? 'is-invalid' : '' }}" type="number" name="ratio" id="ratio" value="{{ old('ratio', '') }}" step="1" required>
                @if($errors->has('ratio'))
                    <div class="invalid-feedback">
                        {{ $errors->first('ratio') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rim_diameter') ? 'is-invalid' : '' }}" type="number" name="rim_diameter" id="rim_diameter" value="{{ old('rim_diameter', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('load_index') ? 'is-invalid' : '' }}" type="number" name="load_index" id="load_index" value="{{ old('load_index', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('speed_rating') ? 'is-invalid' : '' }}" type="text" name="speed_rating" id="speed_rating" value="{{ old('speed_rating', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tire') ? 'is-invalid' : '' }}" name="tire_id" id="tire_id" required>
                    @foreach($tires as $id => $entry)
                        <option value="{{ $id }}" {{ old('tire_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('tire'))
                    <div class="invalid-feedback">
