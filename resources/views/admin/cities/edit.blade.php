            ...
            <form method="POST" action="{{ route("admin.cities.update", [$city->id]) }}" enctype="multipart/form-data">
                ...
                @foreach(siteLanguages() as $locale)
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has($locale.'.name') ? 'is-invalid' : '' }}" type="text"
                               name="{{$locale}}[name]"
                               id="{{$locale}}-name"
                               value="{{ old($locale.'.name', $city->translate($locale, true)->name) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug"
                           id="slug" value="{{ old('slug', $city->slug) }}" required>
                    @if($errors->has('slug'))
                        <div class="invalid-feedback">
                            {{ $errors->first('slug') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('map') ? 'is-invalid' : '' }}" name="map"
                              id="map">{!! old('map', $city->map) !!}</textarea>
                    @if($errors->has('map'))
                        <div class="invalid-feedback">
                            {{ $errors->first('map') }}
                        </div>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="active" id="active"
                               value="1" {{ $city->active || old('active', 0) === 1 ? 'checked' : '' }}>
